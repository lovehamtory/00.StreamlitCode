...
def main() -> None:
    st.set_page_config(page_title="Source Layout 변경 내역", page_icon=":material/difference:", layout="wide")
    ...
    st.markdown('<div class="hero"><h1>✦ Source Layout 변경 내역</h1><p>⚙️ Created by ♡홍율파파♡</p></div>', unsafe_allow_html=True)
    try:
    ...
    if compared:
        if before_label == after_label:
            ...
            try:
                ...
                st.session_state.gp_layout_comparison = {"tables": tables, "columns": columns}
            except Exception as error:
                st.error(str(error), icon=":material/error:")
    if captured:
        try:
