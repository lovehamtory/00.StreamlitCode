...
| `10.Gp2Red/README.md` | 이관 관리 메뉴, Python·Airflow 사전 준비, 최초 투입, 접속·권한·매핑, 검증·배포 경계 |
...
| `MssqlOracleLayoutDiff.py` | MSSQL-Oracle 레이아웃 비교·DDL·컬럼 매핑과 `TB_MIG_TABLE_INFO` 동기화 | `[mssql]`, `[oracle]` |
| `MssqlOracleCountCheck.py` | MSSQL-Oracle 건수·크기 검증과 원천 DB별 성공·실패 현황 | `[mssql]`, `[oracle]` |
...
`10.Gp2Red/app/SrcTgtOrchestrator.py`는 메타가 준비되지 않은 PC에서는 `SrcTgtSetup.py` 초기 설정 화면을 먼저 표시합니다. `.streamlit/migration_setup.toml`은 선택한 스키마명만 보관하는 로컬 Git 제외 파일입니다. `.streamlit/secrets.toml.example`의 `[migration_metadata]`와 `[migration_monitor]`은 각각 `connection_section`과 메타 스키마를 지정하며, 실제 접속값은 참조된 접속 섹션에만 둡니다.
