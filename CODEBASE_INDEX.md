...
- 화면 메뉴는 `접속정보 → 주제영역 → 테이블 레이아웃 → 테이블 변경 비교 → 대상 DDL → SRC·TGT 매핑 → DAG 생성 → 실행 현황 → 검증 → 실행 이력 → EMR → 스냅샷 복구 → 산출물 → 초기 설정` 순서로 항상 모두 표시합니다. 테이블 레이아웃은 선택한 원천 테이블을 가진 매핑 신규·수정으로만 이동하고, 매핑·대상 DDL·DAG 생성은 선택 테이블매핑ID를 유지해 서로 이동합니다. 제목은 탭보다 한 단계 큰 소제목 크기로 통일합니다.
