...
MAPPING_COLUMNS = ["MPG_ID", "SRC_SYSTEM", "SRC_TABLE", "SRC_ENTITY", "TGT_TABLE", "SUM_COLS", "HSH_COLS"]
...
def metadata_settings(target: dict[str, Any]) -> str:
    settings = dict(st.secrets.get("migration_metadata", {}))
    return clean(settings.get("schema")) or clean(target.get("default_schema")) or "mig_meta"
...
def fetch_metadata(target: dict[str, Any]) -> pd.DataFrame:
    schema_name = metadata_settings(target)
    query = f"""
        SELECT T.mpg_id,
               T.src_conn_id,
               T.src_sch_nm || '.' || T.src_tbl_nm,
               '' AS src_entity,
               T.tgt_sch_nm || '.' || T.tgt_tbl_nm,
               COALESCE(LISTAGG(CASE WHEN C.sum_vald_yn THEN C.src_col_nm END, ',') WITHIN GROUP (ORDER BY C.col_ord), '') AS sum_cols,
               COALESCE(LISTAGG(CASE WHEN C.hsh_vald_yn THEN C.src_col_nm END, ',') WITHIN GROUP (ORDER BY C.col_ord), '') AS hsh_cols
          FROM {qualified(schema_name, 'tb_mig_tbl_mpg')} T
          LEFT JOIN {qualified(schema_name, 'tb_mig_col_mpg')} C
            ON C.mpg_id = T.mpg_id
           AND C.active_yn = TRUE
         WHERE T.active_yn = TRUE
           AND T.mpg_sts_cd = 'APPROVED'
           AND T.ddl_aprv_sts_cd = 'APPROVED'
         GROUP BY T.mpg_id, T.src_conn_id, T.src_sch_nm, T.src_tbl_nm, T.tgt_sch_nm, T.tgt_tbl_nm
         ORDER BY T.src_conn_id, T.src_sch_nm, T.src_tbl_nm, T.tgt_sch_nm, T.tgt_tbl_nm
    """
...
def rows(frame: pd.DataFrame) -> list[MappingRow]:
    return [MappingRow(int(row.MPG_ID), upper(row.SRC_SYSTEM), upper(row.SRC_TABLE), clean(row.SRC_ENTITY), upper(row.TGT_TABLE), tuple(item for item in (upper(value) for value in clean(row.SUM_COLS).split(",")) if item), tuple(item for item in (upper(value) for value in clean(row.HSH_COLS).split(",")) if item)) for row in frame.itertuples(index=False)]
...
def comparable(mapping: MappingRow, source: dict[str, Any], target: dict[str, Any]) -> tuple[tuple[str, str, str, str], dict[str, list[str]]]:
    ...
    return (source_schema, source_table, target_schema, target_table), {
        "SUM": [source_columns[key][0] for key in common if upper(source_columns[key][0]) in mapping.sum_columns and source_columns[key][1] in NUMERIC_TYPES and target_columns[key][1] in NUMERIC_TYPES],
        "HASH": [source_columns[key][0] for key in common if upper(source_columns[key][0]) in mapping.hash_columns],
    }
...
def primary_key_columns(values: dict[str, Any], schema_name: str, table_name: str) -> list[str]:
    ...
    with connect(values) as connection:
        with connection.cursor() as cursor:
            ...
            return [clean(row[0]) for row in cursor.fetchall()]
...
def hash_mismatch_rows(values_by_mapping: dict[MappingRow, dict[str, Any]], mappings: list[MappingRow], source: dict[str, Any], target: dict[str, Any], maximum: int = 1000) -> pd.DataFrame:
    ...
    for mapping in mappings:
        ...
        if not mapping.hash_columns or values.get("error") or not values.get("groups", {}).get("HASH"):
            continue
        ...
        try:
            ...
            if [upper(item) for item in source_pk] != [upper(item) for item in target_pk] or not source_pk:
...
def validate(mapping: MappingRow, source: dict[str, Any], target: dict[str, Any]) -> tuple[MappingRow, dict[str, Any]]:
    ...
    try:
        ...
        for enabled_flag, group in ((bool(mapping.sum_columns), "SUM"), (bool(mapping.hash_columns), "HASH")):
            if enabled_flag and not result["groups"][group]:
...
def create_frames(values_by_mapping: dict[MappingRow, dict[str, Any]], mappings: list[MappingRow]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for mapping in mappings:
        ...
        sum_status, sum_bad = group_status("SUM", bool(mapping.sum_columns), values)
        hash_status, _ = group_status("HASH", bool(mapping.hash_columns), values)
        if mapping.hash_columns and not values["error"] and values["groups"]["HASH"]:
...
def render_validation(can_execute: bool) -> None:
    init_state()
    apply_green_style()
    source, target = config("greenplum"), config("redshift_sql")
    controls = st.columns([2, 2, 1, 1, 1])
    with controls[0]:
        mode = st.segmented_control("대상", ["메타데이터", "엑셀"], default="메타데이터", key="vald_mode")
    with controls[1]:
        uploaded = st.file_uploader("매핑 파일", type=["xlsx", "xls", "csv"], key="vald_file") if mode == "엑셀" else None
    with controls[2]:
        workers = st.selectbox("검증 워커", list(range(1, 21)), index=4, key="vald_workers")
    with controls[3]:
        load_clicked = st.button("대상 조회", icon=":material/playlist_add_check:", key="vald_load")
    with controls[4]:
        run_clicked = st.button("검증 실행", icon=":material/play_arrow:", type="primary", disabled=not can_execute, key="vald_run")
    snapshot, target_slot, progress_slot, result_slot = st.empty(), st.empty(), st.empty(), st.empty()
    if load_clicked:
        try:
            if mode == "엑셀" and uploaded is None:
                raise ValueError("엑셀 또는 CSV 파일을 선택하십시오.")
            mappings = load_excel(uploaded) if mode == "엑셀" else fetch_metadata(target)
            st.session_state.gp_rs_mappings = mappings
            st.session_state.gp_rs_results = pd.DataFrame(columns=RESULT_COLUMNS)
            st.session_state.gp_rs_details = pd.DataFrame(columns=DETAIL_COLUMNS)
            st.session_state.gp_rs_hash_mismatches = pd.DataFrame(columns=HASH_MISMATCH_COLUMNS)
            st.session_state.gp_rs_summary = None
            st.session_state.gp_rs_progress = None
        except Exception as error:
            st.error(str(error), icon=":material/error:")
    mappings = st.session_state.gp_rs_mappings
    with snapshot.container():
        render_snapshot(st.session_state.gp_rs_summary, len(mappings))
    if not mappings.empty:
        with target_slot.container():
            st.dataframe(mappings.rename(columns={"MPG_ID": "매핑 ID", "SRC_SYSTEM": "원천 접속", "SRC_TABLE": "원천 테이블", "TGT_TABLE": "대상 테이블", "SUM_COLS": "SUM 검증 컬럼", "HSH_COLS": "HASH 검증 컬럼"}), hide_index=True)
    if st.session_state.gp_rs_progress:
        with progress_slot.container():
            render_progress(st.session_state.gp_rs_progress)
    if run_clicked:
        try:
            if mappings.empty:
                raise ValueError("검증 대상을 먼저 조회하십시오.")
            results, details, hash_mismatches, summary = run_comparison(rows(mappings), source, target, workers, snapshot, progress_slot)
            st.session_state.gp_rs_results = results
            st.session_state.gp_rs_details = details
            st.session_state.gp_rs_hash_mismatches = hash_mismatches
            st.session_state.gp_rs_summary = summary
        except Exception as error:
            st.error(str(error), icon=":material/error:")
...
if __name__ == "__main__":
    ...
    if not allowed(access.authorizations, "EXEC"):
