...
def mapping_frame(query_frame: Callable[..., pd.DataFrame], values: dict[str, Any], schema_name: str, qualified: Callable[[str, str], str], columns: bool = False) -> pd.DataFrame:
    if not columns:
        return query_frame(values, f'''SELECT mpg_id AS "MPG_ID", prj_cd AS "PRJ_CD", sbj_area_cd AS "SBJ_AREA_CD", src_conn_id AS "SRC_CONN_ID", src_sch_nm AS "SRC_SCH_NM", src_tbl_nm AS "SRC_TBL_NM", tgt_conn_id AS "TGT_CONN_ID", tgt_sch_nm AS "TGT_SCH_NM", tgt_tbl_nm AS "TGT_TBL_NM", tgt_tbl_cmt AS "TGT_TBL_CMT", load_sts_cd AS "LOAD_STS_CD", sys_col_nm_arr AS "SYS_COL_NM_ARR", sys_col_fmt_cd AS "SYS_COL_FMT_CD", incr_mthd_cd AS "INCR_MTHD_CD", src_incr_col_nm_arr AS "SRC_INCR_COL_NM_ARR", parl_mthd_cd AS "PARL_MTHD_CD", parl_cnd_arr AS "PARL_CND_ARR", meta_ver_no AS "META_VER_NO"
