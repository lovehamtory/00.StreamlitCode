...
LAYOUTS: dict[str, dict[str, Any]] = {
    "TBL_DFN": {"name": "테이블정의서", "sheet": "테이블정의서", "items": [("MPG_ID", "테이블매핑ID"), ("PRJ_CD", "프로젝트코드"), ("SBJ_AREA_CD", "주제영역코드"), ("SRC_CONN_ID", "원천접속ID"), ("SRC_SCH_NM", "원천스키마명"), ("SRC_TBL_NM", "원천테이블명"), ("TGT_CONN_ID", "대상접속ID"), ("TGT_SCH_NM", "대상스키마명"), ("TGT_TBL_NM", "대상테이블명"), ("TGT_DIST_STYLE", "대상분산방식"), ("TGT_DIST_KEY_COL", "대상분산키"), ("TGT_SORT_STYLE", "대상정렬방식"), ("TGT_SORT_COLS", "대상정렬키"), ("LOAD_STS_CD", "기본적재상태"), ("INCR_BASIS_CD", "증분기준"), ("INCR_BASIS_COL_NM", "증분기준컬럼"), ("PARL_MTHD_CD", "S3병렬방식"), ("PARL_CND_ARR", "S3병렬조건"), ("META_VER_NO", "메타버전번호")]},
    "COL_DFN": {"name": "컬럼정의서", "sheet": "컬럼정의서", "items": [("MPG_ID", "테이블매핑ID"), ("PRJ_CD", "프로젝트코드"), ("SBJ_AREA_CD", "주제영역코드"), ("SRC_SCH_NM", "원천스키마명"), ("SRC_TBL_NM", "원천테이블명"), ("TGT_SCH_NM", "대상스키마명"), ("TGT_TBL_NM", "대상테이블명"), ("COL_ORD", "매핑순서"), ("SRC_COL_NO", "원천컬럼순번"), ("SRC_COL_NM", "원천컬럼명"), ("SRC_DATA_TYPE", "원천데이터타입"), ("SRC_NULL_YN", "원천NULL허용"), ("SRC_KEY_ROLE_CD", "원천키역할"), ("TGT_COL_NO", "대상컬럼순번"), ("TGT_COL_NM", "대상컬럼명"), ("TGT_DATA_TYPE", "대상데이터타입"), ("TGT_NULL_YN", "대상NULL허용"), ("TGT_KEY_ROLE_CD", "대상키역할"), ("TRNSF_EXPR", "변환SQL식"), ("DFLT_EXPR", "기본값SQL식"), ("SUM_VALD_YN", "SUM검증여부"), ("HSH_VALD_YN", "HASH검증여부")]},
    "MPG_DFN": {"name": "매핑정의서", "sheet": "매핑정의서", "items": [("MPG_ID", "테이블매핑ID"), ("PRJ_CD", "프로젝트코드"), ("SBJ_AREA_CD", "주제영역코드"), ("SRC_CONN_ID", "원천접속ID"), ("SRC_SCH_NM", "원천스키마명"), ("SRC_TBL_NM", "원천테이블명"), ("TGT_CONN_ID", "대상접속ID"), ("TGT_SCH_NM", "대상스키마명"), ("TGT_TBL_NM", "대상테이블명"), ("COL_ORD", "매핑순서"), ("SRC_COL_NM", "원천컬럼명"), ("TGT_COL_NM", "대상컬럼명"), ("TGT_DATA_TYPE", "대상데이터타입"), ("TRNSF_EXPR", "변환SQL식"), ("SUM_VALD_YN", "SUM검증여부"), ("HSH_VALD_YN", "HASH검증여부")]},
...
def mapping_frame(query_frame: Callable[..., pd.DataFrame], values: dict[str, Any], schema_name: str, qualified: Callable[[str, str], str], columns: bool = False) -> pd.DataFrame:
    if not columns:
        return query_frame(values, f'''SELECT mpg_id AS "MPG_ID", prj_cd AS "PRJ_CD", sbj_area_cd AS "SBJ_AREA_CD", src_conn_id AS "SRC_CONN_ID", src_sch_nm AS "SRC_SCH_NM", src_tbl_nm AS "SRC_TBL_NM", tgt_conn_id AS "TGT_CONN_ID", tgt_sch_nm AS "TGT_SCH_NM", tgt_tbl_nm AS "TGT_TBL_NM", tgt_dist_style AS "TGT_DIST_STYLE", tgt_dist_key_col AS "TGT_DIST_KEY_COL", tgt_sort_style AS "TGT_SORT_STYLE", tgt_sort_cols AS "TGT_SORT_COLS", load_sts_cd AS "LOAD_STS_CD", incr_basis_cd AS "INCR_BASIS_CD", incr_basis_col_nm AS "INCR_BASIS_COL_NM", parl_mthd_cd AS "PARL_MTHD_CD", parl_cnd_arr AS "PARL_CND_ARR", meta_ver_no AS "META_VER_NO"
                                         FROM {qualified(schema_name, 'tb_mig_tbl_mpg')}
                                        WHERE active_yn = TRUE
                                        ORDER BY prj_cd, sbj_area_cd, mpg_id''')
    return query_frame(values, f'''SELECT T.mpg_id AS "MPG_ID", T.prj_cd AS "PRJ_CD", T.sbj_area_cd AS "SBJ_AREA_CD", T.src_sch_nm AS "SRC_SCH_NM", T.src_tbl_nm AS "SRC_TBL_NM", T.tgt_sch_nm AS "TGT_SCH_NM", T.tgt_tbl_nm AS "TGT_TBL_NM", C.col_ord AS "COL_ORD", C.src_col_no AS "SRC_COL_NO", C.src_col_nm AS "SRC_COL_NM", C.src_data_type AS "SRC_DATA_TYPE", C.src_null_yn AS "SRC_NULL_YN", C.src_key_role_cd AS "SRC_KEY_ROLE_CD", C.tgt_col_no AS "TGT_COL_NO", C.tgt_col_nm AS "TGT_COL_NM", C.tgt_data_type AS "TGT_DATA_TYPE", C.tgt_null_yn AS "TGT_NULL_YN", C.tgt_key_role_cd AS "TGT_KEY_ROLE_CD", C.trnsf_expr AS "TRNSF_EXPR", C.dflt_expr AS "DFLT_EXPR", C.sum_vald_yn AS "SUM_VALD_YN", C.hsh_vald_yn AS "HSH_VALD_YN"
