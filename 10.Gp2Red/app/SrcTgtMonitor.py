...
def latest_table_runs(context: RuntimeContext) -> pd.DataFrame:
    return query_frame(context.values, f'''WITH LAST_RUN AS (
                                            ...
                                            SELECT T.mpg_id, T.prj_cd, A.up_sbj_area_cd, P.sbj_area_nm AS up_sbj_area_nm, T.sbj_area_cd, A.sbj_area_nm, T.src_sch_nm || '.' || T.src_tbl_nm AS src_tbl, T.tgt_sch_nm || '.' || T.tgt_tbl_nm AS tgt_tbl,
                                              ...
                                              JOIN {qualified(context.schema_name, 'tb_mig_sbj_area')} A ON A.sbj_area_cd = T.sbj_area_cd
                                              LEFT JOIN {qualified(context.schema_name, 'tb_mig_sbj_area')} P ON P.sbj_area_cd = A.up_sbj_area_cd
