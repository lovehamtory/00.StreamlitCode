...
TABLE_FIELDS = [
    ...
    "TGT_CONN_ID", "TGT_SCH_NM", "TGT_TBL_NM", "TGT_DIST_STYLE", "TGT_DIST_KEY_COL", "TGT_SORT_STYLE", "TGT_SORT_COLS", "TGT_ENCD_AUTO_YN", "LOAD_MTHD_CD", "WM_COL_NM", "INCR_WHERE_TMPL", "TRNSF_PFL_CD", "S3_STG_PATH", "S3_FILE_FMT_CD",
]
...
FIELD_LABELS = {
    ...
    "TGT_CONN_ID": "대상접속ID", "TGT_SCH_NM": "대상스키마명", "TGT_TBL_NM": "대상테이블명", "TGT_DIST_STYLE": "대상분산방식", "TGT_DIST_KEY_COL": "대상분산키컬럼명", "TGT_SORT_STYLE": "대상정렬방식", "TGT_SORT_COLS": "대상정렬키컬럼목록", "TGT_ENCD_AUTO_YN": "대상자동압축여부", "LOAD_MTHD_CD": "적재방식코드", "WM_COL_NM": "증분기준컬럼명", "INCR_WHERE_TMPL": "증분추출조건템플릿", "TRNSF_PFL_CD": "이관실행프로파일코드", "S3_STG_PATH": "S3스테이징경로", "S3_FILE_FMT_CD": "S3파일형식코드",
...
def defaults(row: dict[str, object]) -> dict[str, object]:
    ...
    output["LOAD_MTHD_CD"] = text(output["LOAD_MTHD_CD"]).upper() or "FULL"
    output["TRNSF_PFL_CD"] = text(output["TRNSF_PFL_CD"]).upper() or "S3_PARQUET"
    output["S3_FILE_FMT_CD"] = text(output["S3_FILE_FMT_CD"]).upper() or "PARQUET"
    for field in ("TGT_DIST_KEY_COL", "TGT_SORT_COLS", "WM_COL_NM", "INCR_WHERE_TMPL", "S3_STG_PATH"):
    ...
    if output["LOAD_MTHD_CD"] not in {"FULL", "INCREMENTAL"}:
        raise ValueError("적재방식코드는 FULL 또는 INCREMENTAL이어야 합니다.")
    if output["S3_FILE_FMT_CD"] not in {"PARQUET", "ICEBERG"}:
        raise ValueError("S3파일형식코드는 PARQUET 또는 ICEBERG여야 합니다.")
    if output["LOAD_MTHD_CD"] == "INCREMENTAL" and (not output["WM_COL_NM"] or not output["INCR_WHERE_TMPL"]):
        raise ValueError("증분 적재에는 증분기준컬럼명과 증분추출조건템플릿이 필요합니다.")
...
def normalized_columns(frame: pd.DataFrame) -> list[dict[str, object]]:
    ...
    for number, source in enumerate(frame.to_dict(orient="records"), start=2):
        ...
        for field in ("SRC_COL_NM", "SRC_DATA_TYPE", "SRC_KEY_ROLE_CD", "TGT_COL_NM", "TGT_DATA_TYPE", "TGT_KEY_ROLE_CD", "TRNSF_EXPR", "DFLT_EXPR"):
...
def upsert_table(cursor: Any, schema_name: str, qualified: Callable[[str, str], str], row: dict[str, object], user_id: str) -> int:
    table_name = qualified(schema_name, "tb_mig_tbl_mpg")
    current_id = mapping_id(cursor, schema_name, qualified, row)
    fields = TABLE_FIELDS[1:]
    columns = [field.lower() for field in fields]
    values = tuple(row[field] for field in fields)
    if current_id is None:
        cursor.execute(
            f"INSERT INTO {table_name} ({', '.join(columns)}, ddl_aprv_sts_cd, mpg_sts_cd, active_yn, crt_by, upd_by) VALUES ({', '.join('%s' for _ in values)}, 'DRAFT', 'DRAFT', TRUE, %s, %s)",
        ...
        return current_id
    cursor.execute(
        f"UPDATE {table_name} SET {', '.join(f'{column} = %s' for column in columns)}, tgt_ddl_sql = NULL, ddl_aprv_sts_cd = 'DRAFT', mpg_sts_cd = 'DRAFT', meta_ver_no = meta_ver_no + 1, upd_by = %s, upd_dtm = GETDATE() WHERE mpg_id = %s",
...
def save_bundle(connect: Callable[[dict[str, Any]], Any], values: dict[str, Any], schema_name: str, qualified: Callable[[str, str], str], tables: list[dict[str, object]], columns: list[dict[str, object]], replace_columns: bool, user_id: str, can_edit: Callable[[str, str], bool]) -> tuple[int, int]:
        ...
        with connection.cursor() as cursor:
            ...
            if replace_columns:
                for map_id in sorted({map_id for map_id, _ in mapped_columns}):
                    cursor.execute(f"UPDATE {table_name} SET active_yn = FALSE, upd_by = %s, upd_dtm = GETDATE() WHERE mpg_id = %s AND active_yn = TRUE", (user_id, map_id))
            for map_id, row in mapped_columns:
                if not replace_columns:
                    cursor.execute(f"UPDATE {table_name} SET active_yn = FALSE, upd_by = %s, upd_dtm = GETDATE() WHERE mpg_id = %s AND col_ord = %s AND active_yn = TRUE", (user_id, map_id, row["COL_ORD"]))
                cursor.execute(
                    f'''INSERT INTO {table_name} (mpg_id, col_ord, src_col_no, src_col_nm, src_data_type, src_null_yn, src_key_role_cd, tgt_col_no, tgt_col_nm, tgt_data_type, tgt_null_yn, tgt_key_role_cd, trnsf_expr, dflt_expr, sum_vald_yn, hsh_vald_yn, active_yn, crt_by, upd_by) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, TRUE, %s, %s)''',
                    (map_id, row["COL_ORD"], row["SRC_COL_NO"], row["SRC_COL_NM"], row["SRC_DATA_TYPE"], row["SRC_NULL_YN"], row["SRC_KEY_ROLE_CD"], row["TGT_COL_NO"], row["TGT_COL_NM"], row["TGT_DATA_TYPE"], row["TGT_NULL_YN"], row["TGT_KEY_ROLE_CD"], row["TRNSF_EXPR"], row["DFLT_EXPR"], row["SUM_VALD_YN"], row["HSH_VALD_YN"], user_id, user_id),
                )
        connection.commit()
...
def source_columns(query_frame: Callable[..., pd.DataFrame], values: dict[str, Any], qualified: Callable[[str, str], str], standard_date: str, owner: str, table: str) -> pd.DataFrame:
    ...
    result["TGT_DATA_TYPE"] = source["SRC_DATA_TYPE"]
...
def render_single(maps: pd.DataFrame, values: dict[str, Any], schema_name: str, user_id: str, can_edit: Callable[[str, str], bool], query_frame: Callable[..., pd.DataFrame], connect: Callable[[dict[str, Any]], Any], qualified: Callable[[str, str], str]) -> None:
    ...
    with st.form("single_mapping_form"):
        ...
        with execution:
            load_mthd_cd = st.selectbox("적재방식코드", ["FULL", "INCREMENTAL"], index=1 if form_value(current, "LOAD_MTHD_CD").upper() == "INCREMENTAL" else 0)
            wm_col_nm = st.text_input("증분기준컬럼명", value=form_value(current, "WM_COL_NM"))
            incr_where_tmpl = st.text_area("증분추출조건템플릿", value=form_value(current, "INCR_WHERE_TMPL"))
            trnsf_pfl_cd = st.text_input("이관실행프로파일코드", value=form_value(current, "TRNSF_PFL_CD", "S3_PARQUET"))
            s3_stg_path = st.text_input("S3스테이징경로", value=form_value(current, "S3_STG_PATH"))
            s3_file_fmt_cd = st.selectbox("S3파일형식코드", ["PARQUET", "ICEBERG"], index=1 if form_value(current, "S3_FILE_FMT_CD").upper() == "ICEBERG" else 0)
        st.markdown("##### 컬럼 매핑")
    ...
    if submitted:
        try:
            ...
            source.update({"MPG_ID": None if current is None else int(selected), "PRJ_CD": prj_cd, "SBJ_AREA_CD": sbj_area_cd, "SRC_CONN_ID": src_conn_id, "SRC_SCH_NM": src_sch_nm, "SRC_TBL_NM": src_tbl_nm, "TGT_CONN_ID": tgt_conn_id, "TGT_SCH_NM": tgt_sch_nm, "TGT_TBL_NM": tgt_tbl_nm, "LOAD_MTHD_CD": load_mthd_cd, "WM_COL_NM": wm_col_nm, "INCR_WHERE_TMPL": incr_where_tmpl, "TRNSF_PFL_CD": trnsf_pfl_cd, "S3_STG_PATH": s3_stg_path, "S3_FILE_FMT_CD": s3_file_fmt_cd})
            ...
            save_bundle(connect, values, schema_name, qualified, [table], columns, True, user_id, can_edit)
...
def render_upload(values: dict[str, Any], schema_name: str, user_id: str, can_edit: Callable[[str, str], bool], connect: Callable[[dict[str, Any]], Any], qualified: Callable[[str, str], str]) -> None:
    ...
    if st.button("일괄 반영", icon=":material/upload:", type="primary"):
        try:
            ...
            saved_tables, saved_columns = save_bundle(connect, values, schema_name, qualified, tables, columns, replace_columns, user_id, can_edit)
...
def render_mapping_workspace(maps: pd.DataFrame, values: dict[str, Any], schema_name: str, user_id: str, can_edit: Callable[[str, str], bool], query_frame: Callable[..., pd.DataFrame], connect: Callable[[dict[str, Any]], Any], qualified: Callable[[str, str], str]) -> None:
    mode = st.segmented_control("매핑 업무", ["📝 단건 등록·수정", "📤 일괄 업로드"], default="📝 단건 등록·수정", label_visibility="collapsed")
    if mode == "📝 단건 등록·수정":
        render_single(maps, values, schema_name, user_id, can_edit, query_frame, connect, qualified)
    else:
        render_upload(values, schema_name, user_id, can_edit, connect, qualified)
