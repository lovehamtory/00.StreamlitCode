...
Redshift 대상 접속은 한 건만 등록합니다. S3는 별도 `CONN_ID`가 아니며, 대상 접속의 S3 기준경로 아래에 FULL·INCR 기준본을 생성합니다.

## 6. 주제영역

주제영역은 코드·업무명·원천접속ID·대상접속ID를 함께 관리합니다. 예를 들어 상위 `A01` 아래에 `A010001`~`A010010`을 등록합니다. 테이블 매핑은 실제 DAG를 만들 하위 주제영역에 연결하며 접속정보는 중복 입력하지 않습니다.
...
| 상위주제영역 | 선택값, 분류·표시용 |
| 원천접속ID | 해당 주제영역의 SRC 연결 |
| 대상접속ID | 해당 주제영역의 TGT 연결 및 S3 기준경로 |
| 표시순서 | 화면 정렬용 |
