...
접속정보는 방향을 갖지 않습니다. 한 접속이 어떤 매핑에서는 원천이고 다른 매핑에서는 대상이 될 수 있으므로, 방향은 테이블 매핑의 `SRC_CONN_ID`, `TGT_CONN_ID`가 결정합니다.
...
Redshift 대상 접속은 한 건만 등록합니다. 그 접속의 S3 기준경로 아래에 프로젝트·주제영역·원천 스키마·테이블 경로가 만들어집니다. S3는 별도 `CONN_ID`가 아닙니다.

## 6. 주제영역

주제영역은 코드를 고정해 두고 업무명만 관리합니다. 예를 들어 상위 `A01` 아래에 `A010001`~`A010010`을 등록합니다. 테이블 매핑은 실제 DAG를 만들 하위 주제영역에 연결합니다.

| 항목 | 설명 |
| --- | --- |
| 주제영역코드 | `A01`, `A010001` 등 영문·숫자·밑줄 코드 |
| 주제영역명 | 카드, 고객, 매출 등 업무명 |
| 상위주제영역 | 선택값, 분류·표시용 |
| 표시순서 | 화면 정렬용 |
...
테이블과 컬럼은 같은 화면에서 등록·수정합니다. 신규 매핑은 수집한 원천 레이아웃에서 시작하고, 필요하면 대상 구조 자동 반영을 사용합니다. 일괄 업로드는 테이블매핑·컬럼매핑 두 시트를 한 Excel 파일로 입력합니다.
